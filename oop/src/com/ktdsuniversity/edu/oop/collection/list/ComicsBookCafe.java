...
class ComicsBookCafe {
	...
	/** 만화 카페에 있는 책 목록 */
	private List<ComicsBook> bookList;
	
	// 생성자
	ComicsBookCafe(int money){
		this.money = money;
	...
	void printBookList() {
		...
		for(int i = 0; i < this.bookList.size(); i++) {
			...
			if(this.bookList.get(i) != null) {
				
				ComicsBook book = this.bookList.get(i);
	...
	int getPrice(int index) {
		if(this.bookList.get(index) != null) {
			return this.bookList.get(index).getPrice();
		} else {
			return -1;
		}
	}
	...
	void rental(Visitor visitor, int index) {
		...
		if(this.bookList.get(index).isRental()) {
			System.out.println("이미 대여됨");
			return;
		}
		
		int price = this.bookList.get(index).getPrice();
		...
		if(visitor.getMoney() >= price) {
			...
			this.bookList.get(index).rentalBook();
